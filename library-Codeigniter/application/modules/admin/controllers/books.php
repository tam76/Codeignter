...
class Books extends Define{
    ...
    function index($order = 'bookid', $by = 'DESC', $offset = 0){
        ...
        $config['base_url']         = base_url('index.php/admin/books/index/'.$order.'/'.$by); // xác định trang phân trang 
        $config['total_rows']       = $this->book->Countbook(); // xác định tổng số record 
        $config['per_page']         = BOOK_LIMIT; // xác định số record ở mỗi trang 
    ...
    function add(){
        ...
        if($this->input->post('btnBookAdd')){
            $this->form_validation->set_rules('txtTitle', 'Tiêu đề', 'required|min_length[5]|max_length[250]|is_unique[books.book_title]');
            $this->form_validation->set_rules('txtAuthor', 'Tác giả', 'min_length[3]|max_length[250]');
            $this->form_validation->set_rules('txtPublisher', 'Nhà xuất bản', 'min_length[3]|max_length[250]');
            ...
            if ($this->form_validation->run() == FALSE){
                ...
                if($this->_uploadImg($name) == false){
                    ...
                    $book_url = $name.'.swf';
                    exec(FCPATH.'tool\pdf2swf.exe '.BOOK_UPOAD_PATH.$name.'.pdf -o '.BOOK_UPOAD_PATH.$book_url.' -f -T 9 -t -s  storeallcharacters');
